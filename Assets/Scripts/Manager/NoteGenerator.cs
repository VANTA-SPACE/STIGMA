...
namespace Manager {
    public class NoteGenerator : MonoBehaviour
    {
        ...
        public void PrepareGeneratingNotes(List<NoteData> datas) {
            ...
            datas.Sort((data, noteData) => data.StartBeat < noteData.StartBeat ? 1 : data.StartBeat < noteData.StartBeat ? -1 : 0);
        ...
        public IEnumerator GenerateNotesCo() {
            while (NoteDatas.Count > 0 && GameManager.Instance.IsPlayingLevel) {
                while (NoteDatas.Count > 0 && NoteDatas.Peek().StartBeat + NoteGenerationBeat <= GameManager.Instance.CurrentBeat) {
